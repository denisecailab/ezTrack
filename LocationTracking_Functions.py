...
def LoadAndCrop(video_dict, cropmethod=None, fstfile=False, accept_p_frames=False):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
...
def Reference(video_dict, num_frames=100, altfile=False, fstfile=False, frames=None):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    cap_max = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    cap_max = int(video_dict['end']
                  ) if video_dict['end'] is not None else cap_max
    ...
    for (idx, framenum) in enumerate(frames):
        ...
        while not grabbed:
            ...
            if ret:
                ...
                framenum = np.random.randint(
                    video_dict['start'], cap_max, 1)[0]
                pass
    cap.release()
    ...
    image = hv.Image((np.arange(reference.shape[1]),
                      ...
                      reference)).opts(width=int(reference.shape[1]*video_dict['stretch']['width']),
                                       height=int(
                                           reference.shape[0]*video_dict['stretch']['height']),
                                       invert_yaxis=True,
                                       cmap='gray',
                                       colorbar=True,
                                       toolbar='below',
...
def Locate(cap, tracking_params, video_dict, prior=None):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    if prior is not None and tracking_params['use_window']:
    ...
    if ret:
        ...
        if tracking_params['method'] == 'abs':
            ...
            dif = np.absolute(frame-video_dict['reference'])  # default
        ...
        if prior is not None and tracking_params['use_window']:
            ...
            dif_weights[slice(ymin if ymin > 0 else 0, ymax),      # type: ignore
                        slice(xmin if xmin > 0 else 0, xmax)] = 1  # type: ignore
            dif = dif*dif_weights
...
def TrackLocation(video_dict, tracking_params):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    for f in tqdm(range(len(D))):
        ...
        if ret:
            if 'com' not in locals():
                raise ValueError("'com' is unbound.")

            Y[f] = com[0]  # type: ignore
            X[f] = com[1]  # type: ignore
            if f > 0:
...
def LocationThresh_View(video_dict, tracking_params, examples=4):
    ...
    for _ in range(examples):
        ...
        while not ret:
            # select random frame
            frm = np.random.randint(video_dict['start'], cap_max)
            # sets frame to be next to be grabbed
            cap.set(cv2.CAP_PROP_POS_FRAMES, frm)
            ret, dif, com, frame = Locate(cap, tracking_params, video_dict)
        else:
            if any([v not in locals() for v in ('ret', 'dif', 'com', 'frame')]):
                raise ValueError

        # plot original frame
        image_orig = hv.Image(
            (np.arange(frame.shape[1]), np.arange(frame.shape[0]), frame))  # type: ignore
        image_orig.opts(
            width=int(video_dict['reference'].shape[1]
                      * video_dict['stretch']['width']),
            height=int(video_dict['reference'].shape[0]
                       * video_dict['stretch']['height']),
            invert_yaxis=True, cmap='gray', toolbar='below',
            title="Frame: " + str(frm))  # type: ignore

        ps = hv.Points(([com[1]], [com[0]])).opts(  # type: ignore
            color='red', size=20, marker='+', line_width=3)
        orig_overlay = image_orig * ps.opts(
            color='red', size=20, marker='+', line_width=3)

        # plot heatmap
        dif = dif*(255//dif.max())  # type: ignore
        image_heat = hv.Image((
        ...
        image_heat.opts(
            ...
            title="Frame: " + str(frm - video_dict['start']))  # type: ignore
        heat_overlay = image_heat * ps
...
def ROI_plot(video_dict):
    ...
    nobjects = len(video_dict['region_names']
                   ) if video_dict['region_names'] else 0
...
def ROI_Location(video_dict, location):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    for poly in range(len(video_dict['roi_stream'].data['xs'])):
        x = np.array(video_dict['roi_stream'].data['xs']
                     [poly])  # x coordinates
        y = np.array(video_dict['roi_stream'].data['ys']
                     [poly])  # y coordinates
        ...
        # save to ROI masks as boolean
        ROI_masks[video_dict['region_names'][poly]] = mask == 255
...
def Batch_Process(video_dict, tracking_params, bin_dict, accept_p_frames=False):
    ...
    for file in video_dict['FileNames']:
        ...
        location.to_csv(os.path.splitext(video_dict['fpath'])[
                        0] + '_LocationOutput.csv', index=False)
        file_summary = Summarize_Location(
            location, video_dict, bin_dict=bin_dict)

        summary_alls.append(file_summary)

        trace = showtrace(video_dict, location)
        heatmap = Heatmap(video_dict, location, sigma=None)
        images = images + [(trace.opts(title=file)),
                           (heatmap.opts(title=file))]

    summary_all = pd.concat(summary_alls, sort=False)

    # Write summary data to csv file
    sum_pathout = os.path.join(os.path.normpath(
        video_dict['dpath']), 'BatchSummary.csv')
...
def PlayVideo(video_dict, display_dict, location):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    if display_dict['save_video']:
        ...
        # cv2.VideoWriter_fourcc(*'jpeg') #only writes up to 20 fps, though video read can be 30.
        fourcc = 0
...
def PlayVideo_ext(video_dict, display_dict, location, crop=None):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
...
def showtrace(video_dict, location, color="red", alpha=.8, size=3):
    ...
    image = hv.Image((np.arange(video_dict['reference'].shape[1]),
                      ...
                      video_dict['reference'])
                            ...
                            height=int(
                                video_dict['reference'].shape[0]*video_dict['stretch']['height']),
                            invert_yaxis=True, cmap='gray', toolbar='below',
                            title="Motion Trace")
...
def Heatmap(video_dict, location, sigma=None):
    ...
    sigma = np.mean(heatmap.shape)*.05 if sigma is None else sigma
...
def DistanceTool(video_dict):
    ...
    points = hv.Points([]).opts(
        active_tools=['point_draw'], color='red', size=10)
    ...
    def markers(data, distance):
        ...
        if len(x_ls) > 1:
            ...
            return hv.Labels((x_ctr, y_ctr, text)).opts(
                text_color='blue', text_font_size='14pt')
        else:
            return hv.Labels("").opts(
                text_color='blue', text_font_size='14pt')
...
def ScaleDistance(video_dict, df=None, column=None):
    ...
    if video_dict['scale']['px_distance'] is not None:
        video_dict['scale']['factor'] = video_dict['scale']['true_distance'] / \
            video_dict['scale']['px_distance']
...
def Mask_select(video_dict, fstfile=False):  # noqa: C901
    """
    -------------------------------------------------------------------------------------
    ...
    def make_mask(data, mask):
        ...
        if len(x_ls) > 0:
            ...
            for submask in range(len(x_ls)):
                x = np.array(mask['stream'].data['xs']
                             [submask])  # x coordinates
                y = np.array(mask['stream'].data['ys']
                             [submask])  # y coordinates
                xy = np.column_stack((x, y)).astype(
                    'uint64')  # xy coordinate pairs
...
def check_p_frames(cap, p_prop_allowed=.01, frames_checked=300):
    ...
    frames_checked = min(frames_checked, int(
